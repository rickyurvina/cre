...
@section('subheader')
    ...
    <div class="subheader-block d-lg-flex align-items-center">
        @can('admin-crud-admin')
            <a href="{{ route('thresholds.create') }}" class="btn btn-success btn-sm"><span class="fas fa-plus mr-1"></span> &nbsp;{{ trans('general.add_new') }}</a>
        @endcan
    </div>
@endsection
...
@section('content')
    ...
    <div class="card">
        <x-search route="{{ route('thresholds.index') }}"/>
        <div class="table-responsive">
            <table class="table m-0" id="table_thresholds">
                ...
                @foreach($Thresholds as $item)
                    <tr>
                        <th class="d-none"></th>
                        <td><a href="{{ route('thresholds.show', $item->id) }}">{{ $item->name }}</a></td>
                        <td class="text-center w-20">
                                <form action="{{ route('thresholds.destroy', $item->id) }}" method="POST">
                                    @can('admin-crud-admin')
                                        <a class="mr-2" href="{{ route('thresholds.edit', $item->id) }}" data-toggle="tooltip" data-placement="top" title="" data-original-title="Editar"><i
