...
@section('subheader-title')
    <i class="fal fa-plus text-primary"></i> {{ trans('general.title.edit', ['type' => trans_choice('general.units', 1)]) }}
    - {{ $unit->name }}
@endsection
...
@section('content')
    <div class="card">
        ...
        <form action="{{ route('indicator_units.update', $unit->id) }}" method="post">
            @csrf
            @method('PUT')
            <div class="card-body">
                <div class="row">
                    <div class="form-group col-6">
                        ...
                        <div class="input-group bg-white shadow-inset-2">
                            ...
                            <input type="text" required name="name" id="name" class="form-control border-left-0 bg-transparent pl-0 @error('name') is-invalid @enderror"
                                   value="{{ old('name', $unit->name) }}" placeholder="{{ trans('general.form.enter', ['field' => trans('general.name')]) }}">
                    ...
                    <div class="form-group col-6">
                        ...
                        <label class="form-label" for="name">{{ trans('indicators.indicator.abbreviation')  }}</label>
                        <div class="input-group bg-white shadow-inset-2">
                            <div class="input-group-prepend">
                                    <span class="input-group-text bg-transparent border-right-0">
                                        <i class="fal fa-sort-numeric-down-alt"></i>
                            ...
                            <input type="text" required name="abbreviation" id="abbreviation"
                                   ...
                                   placeholder="{{ trans('general.form.enter', ['field' => trans('general.abbreviation')]) }}">
                            <div class="invalid-feedback">{{ $errors->first('abbreviation',':message') }} </div>
                        </div>
                    </div>
                </div>
            </div>
