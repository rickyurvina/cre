...
@section('title', trans('general.title.create', ['type' => trans_choice('general.sources', 1)]))

@section('subheader-title')
    <i class="fal fa-plus text-primary"></i> {{ trans('general.title.create', ['type' => trans_choice('general.sources', 1)]) }}
@endsection

@section('content')
    <div class="card">
        <form action="{{ route('indicator_sources.store') }}" method="post">
            @csrf
            <div class="card-body">
                <div class="row">
                    ...
                    <div class="form-group col-6 required">
                        ...
                        <div class="input-group bg-white shadow-inset-2">
                            ...
                            <input type="text" name="institution" id="institution"
                                   ...
                                   value="{{ old('institution') }}" placeholder="{{ trans('general.form.enter', ['field' => trans('general.institution')]) }}">
                    ...
                    <div class="form-group col-lg-6 required">
                        ...
                        <div class="input-group bg-white shadow-inset-2">
                            ...
                            <select class="custom-select @error('type') is-invalid @enderror" id="type" name="type">
                                ...
                                <option value="{{$IndicatorSource::TYPE_SURVEY}}" {{ old("type") == $IndicatorSource::TYPE_SURVEY ? "selected":"" }}>{{trans('indicators.indicator.TYPE_'.$IndicatorSource::TYPE_SURVEY) }}</option>
                                <option value="{{$IndicatorSource::TYPE_ADMINISTRATIVE_RECORD}}" {{ old("type") == $IndicatorSource::TYPE_ADMINISTRATIVE_RECORD ? "selected":"" }}>{{ trans('indicators.indicator.TYPE_'.$IndicatorSource::TYPE_ADMINISTRATIVE_RECORD) }}</option>
                                <option value="{{$IndicatorSource::TYPE_TRANSACTIONAL}}" {{ old("type") == $IndicatorSource::TYPE_TRANSACTIONAL ? "selected":"" }}>{{trans('indicators.indicator.TYPE_'.$IndicatorSource::TYPE_TRANSACTIONAL) }}</option>
                            </select>
