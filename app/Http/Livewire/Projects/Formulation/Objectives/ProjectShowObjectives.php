...
class ProjectShowObjectives extends TableComponent
{
    ...
    {
        $results = Task::when(count($this->selectedObjectives) > 0, function (Builder $query) {
            $query->whereIn('objective_id', $this->selectedObjectives);
        })->orderBy('objective_id', 'asc')
            ->where('type','!=','task')
            ->whereIn('objective_id', $this->project->objectives->pluck('id'))
            ->search('text', $this->search)
            ->paginate(setting('default.list_limit', '25'));
        return view('livewire.projects.formulation.objectives.project-show-objectives', compact('results'));
    }
